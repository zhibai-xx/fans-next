...
const OptimizedImageCard: React.FC<ImageCardProps> = ({
  ...
  return (
    <Card
      className="group cursor-pointer transition-all duration-300 hover:shadow-xl hover:-translate-y-1 overflow-hidden border-0 bg-white dark:bg-gray-800 break-inside-avoid mb-4 rounded-2xl"
      onClick={onClick}
...
const ActionButton: React.FC<ActionButtonProps> = ({
  ...
  return (
    <Button
      variant="ghost"
      size="sm"
      className={`h-8 w-8 p-0 rounded-full bg-white/80 transition-all duration-200 ${className} ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
      title={tooltip}
      disabled={loading}
      onClick={(e) => {
        e.stopPropagation();
